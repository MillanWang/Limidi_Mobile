...
export const ALL_COLOR_SERIES = { //Not gonna be used directly with new elemental approach
    ...
    cyan: ["#e1f5fe", "#b3e5fc", "#81d4fa", "#4fc3f7", "#29b6f6", "#03a9f4", "#039be5", "#0288d1", "#0277bd", "#01579b",],
...
export function invertHex(hex: string) {
    ...
    function padZero(str: string, len = 2) { return (new Array(len).join('0') + str).slice(-len); }
    function getColorComponent(hex: string, n: number) { return (255 - parseInt(hex.slice(n, n + 2), 16)).toString(16) }

    if (hex.indexOf('#') === 0) { hex = hex.slice(1); }
    if (hex.length === 3) { hex = hex[0] + hex[0] + hex[1] + hex[1] + hex[2] + hex[2]; }
    if (hex.length !== 6) { throw new Error('Invalid HEX color.'); }

    return '#' + padZero(getColorComponent(hex, 0)) + padZero(getColorComponent(hex, 2)) + padZero(getColorComponent(hex, 4));
}
